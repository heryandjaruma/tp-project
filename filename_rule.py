...
def export_particle_label_only(fold, room, mix, label_utama, increment):
    return "_".join([str(fold), str(room), str(mix),str(label_utama), '%03d' % (increment)]) + '.wav'

def export_particle_label_overlap(fold, room, mix, label_utama, label_kedua):
    return "_".join([str(fold), str(room), str(mix), 'ov2',str(label_utama), str(label_kedua)]) + '.wav'